...
Khách chọn 1 trong các lối vào:
      ...
      │       
      │     → Click → set customer_id cố định vào cookie (VD customer_id=demo_001) → vào thẳng chat
      │       
      │
      └── Khách thật (không qua demo)
            → Chưa có customer_id → sinh UUID mới, lưu cookie/localStorage, dùng lại cho các lần quay lại cùng trình duyệt
              
      ↓
Mỗi lượt hội thoại → sau khi xử lý xong (slot-filling, RAG, trả lời):
      → Ghi vào bảng `customer_memory` (Postgres/SQLite cho demo):
